// OpenGLLight.cpp : アプリケーションのエントリ ポイントを定義します。
//
...
protected:
	...
	//	glLightfv( GL_LIGHT0, GL_POSITION, lightPostion );
		glLightfv( GL_LIGHT0, GL_POSITION, m_Light.pt );
...
public:
	...
	void	Display( void ){
		...
		glPushMatrix();
		glTranslatef( m_Object.X(), m_Object.Y(), m_Object.Z() );
...
public:
	void	OnKeyDown( UINT uVk, BOOL fDown, int nRepeat, UINT uFlags ){
		const float fDistance	= 0.05f;
		switch( uVk ){
		case VK_UP:
			m_Object.Y() += fDistance;
			Invalidate();
			break;
		case VK_DOWN:
			m_Object.Y() -= fDistance;
