...
public:
	...
	BOOL			GetItemText( HTREEITEM hItem, LPTSTR lpszBuf, UINT nNum ){
		TVITEM		item = {0};
		item.mask			= TVIF_HANDLE | TVIF_TEXT;
		item.hItem			= hItem;
		item.pszText		= lpszBuf;
		item.cchTextMax		= nNum;
		return GetItem( &item );
	}

	HTREEITEM		HitTest( LPTVHITTESTINFO pHit ){
		return TreeView_HitTest(m_hWnd, pHit);
	}
	...
	void			SetCheckState( HTREEITEM hItem, BOOL bCheck ){
		TreeView_SetCheckState(m_hWnd, hItem, bCheck);
	}
