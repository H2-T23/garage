...
namespace GENERIC {
	...
	public:
		...
		THandle( void ) : m_handle(DEFAULT) {
		}
		explicit THandle( const CLASS& obj ) : m_handle(DEFAULT) {
			SetHandle( obj );
		}
		explicit THandle( const TYPE& handle ) : m_handle(DEFAULT) {
			SetHandle( handle );
		}
		...
		bool	operator==( const CLASS& obj ) const {
			return(m_handle == obj.m_handle);
		}
		bool	operator!=( const CLASS& obj ) const {
			return !(*this == obj);
		}

		operator TYPE(){ return(m_handle); }
	};
